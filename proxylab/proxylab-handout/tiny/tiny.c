...
void doit(int fd) 
{
    ...
    if (strcasecmp(method, "GET")) {                     // 目前仅支持get方法
    ...
    if (stat(filename, &sbuf) < 0) {                     // 文件未找到
	    clienterror(fd, filename, "404", "Not found","Tiny couldn't find this file");
	    return;
    }                                                    //line:netp:doit:endnotfound

    if (is_static) { /* Serve static content */          
        if (!(S_ISREG(sbuf.st_mode)) || !(S_IRUSR & sbuf.st_mode)) { // 如果当前用户无读取权限
        ...
        if (!(S_ISREG(sbuf.st_mode)) || !(S_IXUSR & sbuf.st_mode)) { // 如果当前用户无读取权限
...
int parse_uri(char *uri, char *filename, char *cgiargs)  // 解析url
{
    ...
    else {  /* Dynamic content */                        // 解析动态资源
        ptr = index(uri, '?');                           
        if (ptr) {                                       // 如果有参数
            strcpy(cgiargs, ptr+1);
            *ptr = '\0';
	    }
	    else strcpy(cgiargs, "");                        // 如果没有参数
