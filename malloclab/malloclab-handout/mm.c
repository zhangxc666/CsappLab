...
static void *coalesce(void *bp)
{
    ...
    { // 下一块是空闲块
        ...
        if(NEXT_BLKP(bp)==next_fit_bp)next_fit_bp=bp; // 当合并的块正好是下一次查询需要查询的块时
    ...
    { // 上一块是空闲块
        ...
        if(bp==next_fit_bp)next_fit_bp=PREV_BLKP(bp); // 当合并的块正好是下一次查询需要查询的块时
    ...
    { // 上下两块均是空闲块
        ...
        if(bp==next_fit_bp || NEXT_BLKP(bp)==next_fit_bp)next_fit_bp=PREV_BLKP(bp);// 当合并的块正好是下一次查询需要查询的块时
        bp = PREV_BLKP(bp);                   // 修改bp为上一块的位置
    }
    return bp;
}
static void *find_fit(size_t new_size)// Perf index = 44 (util) + 23 (thru) = 67/100
{
    ...
    for (bp = heap_listp; GET_SIZE(HDRP(bp)) > 0; bp = NEXT_BLKP(bp))
    {
        ...
        {
            // printf("--打印操作：进行 find_fit 操作，找到大于等于 %d 大小的块，位置是在 %p \n",new_size,bp);
            return bp;
        }
    }
    // printf("--打印操作：进行 find_fit 操作，未找到大于 %d 大小的块\n",new_size);
    return NULL;
}
