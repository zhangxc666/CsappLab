 ...
 *   Rating: 4
 */
unsigned floatScale2(unsigned uf) // 题目意思，将uf解释成float，并将其乘2返回，二进制表示用unsigned表示
{
   unsigned flag = (uf >> 31) & 1;     // 符号位
   unsigned exp = (uf >> 23) & (0xFF); // 阶码
   unsigned fac = uf & (0x7fffff);     // 尾数
   if (exp == 255)
      return uf; // 当此时是NAN直接返回
   if (exp == 0)
      return (flag << 31) | (exp << 23) | (fac << 1); // 当其为非规格化小数时
   exp++;
   return (flag << 31) | (exp << 23) | fac;
}
/*
...
int floatFloat2Int(unsigned uf)
{
   unsigned flag = (uf >> 31) & 1;                // 符号位
   unsigned exp = (uf >> 23) & (0xFF);            // 阶码
   unsigned fac = (uf & (0x7fffff)) | (0xffffff); // 尾数
   int t = exp - 127 - 23;                        // 与指数23进行比较
   if (exp >= (31 + 127))
      return 0x80000000; // 当指数>31时都返回0x8000000，容易证明。（当exp=31+127 且 fac=0 且 flag=0时，刚好解释为(int)0x8000000)
   if (exp < 127)
      return 0; // 小于127指指数为负，此时必定都是小数，则返回为0
   if (t < 0)
      fac = (fac >> (-t)); // 指数在0~30范围内，此时可以正常转换，考虑当指数小于23时，此时需要舍入尾数，即进行右移；
   else
      fac = (fac << t); // 大于或等于23无需舍入左移即可
   if (flag)
      fac = ~fac + 1; // 如果此时是负数需要进行取反
...
/*
   本题进行分类讨论：
      一、当x在[-126,127]范围内时，只需构造exp=x+127即可 
      二、当x在[-149,-127]范围内时，只需构造fac=1<<(23+x+126); 此时是非规格化小数
      三、当x在[-INF,-149)范围内时，数字过小无法表示，返回0;
      四、当x在(126,INF)范围时，数字过大无法表示，返回0x7f800000;
   本题有个坑点 最大的可表示的数不是2^127-2^103，这是最大的规格化数。
               最大的数应该是 0 11111110 00000000000000000000000 即(2-0)*2^126=2^127，这是非规格化的
*/
unsigned floatPower2(int x)
{
   if (x > 127)return 0x7f800000;
   if (x >= -126)return (x + 127) << 23;
   else if (x >= -149) return 1 << (23 + x + 126);
   return 0;
}
